...
export class RealDataService {
  ...
  static async getEmployeeDirectory(filters?: {
    ...
    try {
      ...
      console.error('Error fetching employee directory:', error?.message || error)

      // Check if it's a network error and use offline data
      const errorMessage = error?.message || error?.toString() || ''
      if (errorMessage.toLowerCase().includes('failed to fetch') ||
          errorMessage.toLowerCase().includes('network')) {
        console.log('📴 Using offline employee directory due to network issues')
  ...
  static async getEmployeeStats() {
    try {
      ...
      if (errorMessage.toLowerCase().includes('failed to fetch') ||
          errorMessage.toLowerCase().includes('network')) {
        console.log('📴 Using offline employee stats due to network issues')
  ...
  static async getDashboardData() {
    try {
      ...
      if (errorMessage.toLowerCase().includes('failed to fetch') ||
          errorMessage.toLowerCase().includes('network')) {
        console.log('📴 Using offline dashboard data due to network issues')
