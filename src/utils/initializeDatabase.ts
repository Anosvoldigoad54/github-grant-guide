...
export async function createSampleUserProfile(
  ...
  try {
    console.log('🔧 Creating sample user profile for:', email)
    ...
    console.error('❌ Error in createSampleUserProfile:', error)
    return {
      success: false,
...
async function createInitialUserPreferences(
  ...
  try {
    ...
    if (error) {
      console.warn('⚠️ Failed to create user preferences:', error)
    } else {
      console.log('✅ User preferences created')
    }
  } catch (error) {
    console.warn('⚠️ Error creating user preferences:', error)
  }
}
...
async function createInitialLeaveBalances(employeeId: string): Promise<void> {
  try {
    ...
    if (error) {
      console.warn('⚠️ Failed to create leave balances:', error)
    } else {
      console.log('✅ Leave balances created')
    }
  } catch (error) {
    console.warn('⚠️ Error creating leave balances:', error)
  }
}
...
export async function initializeCompleteDatabase(): Promise<InitializationResult> {
  try {
    ...
    if (failedResults.length > 0) {
      console.warn('⚠️ Some initialization steps failed:', failedResults)
      return {
        success: false,
