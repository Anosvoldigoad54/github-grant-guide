...
export const AuthGuard: React.FC<AuthGuardProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log('🔐 Starting security verification with profile loaded')
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log('🔐 Setting profile loading timeout fallback')
      timeoutId = setTimeout(() => {
        if (mountedRef.current && !hasVerified && !verificationRef.current) {
          console.log('⚠️ Profile loading timeout - proceeding with basic verification')
          verificationRef.current = true
          performEnhancedSecurityVerification()
        }
  ...
  const performEnhancedSecurityVerification = async () => {
    ...
    try {
      ...
      console.log('🔐 Starting enhanced security verification...')
      ...
      if (!profile && !allowPartialAccess) {
        console.warn('⚠️ Profile not loaded - allowing access with basic verification only')
      ...
      if (profile) {
        ...
        if (checkProbationStatus && profile.probation_end_date) {
          ...
          if (now < probationEnd) {
            ...
            console.warn('⚠️ User on probation, access may be limited')
          }
        }
      ...
      if (sessionHealth === 'critical') {
        console.warn('⚠️ Critical session health detected')
      ...
      if (securityContext) {
        ...
        if (securityContext.risk_level === 'high') {
          console.warn('🚨 High security risk detected')
      ...
      console.log('✅ Enhanced security verification completed successfully')
      
    } catch (error: any) {
      console.error('❌ Enhanced security verification failed:', error.message)
  ...
  const verifyEnhancedPermissions = (permissions: string[]): boolean => {
    if (!profile?.role?.permissions) {
      console.warn('⚠️ No permissions found in profile')
      return false
    }

    // System admin users have all permissions
    if (profile.role.permissions.includes('*')) {
      console.log('✅ System admin - all permissions granted')
      return true
    }
    ...
    if (missingPermissions.length === 0) {
      ...
      console.warn('⚠️ Missing required permissions:', missingPermissions)
      return false
    }
  }
  ...
  if (isAuthenticated && user && verificationStatus === 'idle') {
    console.log('🔐 User authenticated but verification pending, showing loading...')
    return fallback || (
      <Box
        sx={{
          minHeight: '100vh',
